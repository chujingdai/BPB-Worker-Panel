...
export async function getDataset(request, env) {
    ...
    try {
        proxySettings = await env.anan.get("proxySettings", {type: 'json'});
        warpConfigs = await env.anan.get('warpConfigs', {type: 'json'});
    } catch (error) {
        console.log(error);
...
export async function updateDataset (request, env) {
    ...
    if (!isReset) {
        try {
            currentSettings = await env.anan.get("proxySettings", {type: 'json'});
        } catch (error) {
            console.log(error);
    ...
    try {    
        await env.anan.put("proxySettings", JSON.stringify(proxySettings));
